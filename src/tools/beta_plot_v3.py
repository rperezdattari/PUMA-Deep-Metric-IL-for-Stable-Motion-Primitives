...
plt.plot(t, y, label='$d(t)$', linewidth=2, color='black')
...
for i, t_i in enumerate(t):
    ...
    if i == 0:
        ...
        m = (y_upper[i-1] - max_b) / delta
        y_upper[i] = y_upper[i-1] - m * time_step

plt.plot(t, y_upper, label='$\\beta(t)$', linewidth=2, color='C0', linestyle='--')
#plt.plot(t, np.array(max_b_list), label='$max_{b}$', linewidth=2, color='C1', linestyle='-')
...
for idx, init_cond in enumerate(initial_conditions):
    ...
    if i == 0:
        plt.scatter(t_samples, y_samples, color='C3', label='$d(n \cdot \Delta t + \\alpha)$', s=20, zorder=100)
    else:
